...
def display_everything(direction, name, update=False, readied=False):
    ...
    if readied and not restarting:
        ...
        draw.text((38, 12 - 7), name_line[0], font=LARGE_FONT, fill=GREEN)
        draw.rectangle([38, 38, 38 + width(name_line[0], LARGE_FONT), 38], fill=GREEN)

        # draw location
        draw.text((38, 43), location, font=MEDIUM_FONT, fill=GREEN)

        # draw info
        y_offset = 0
        for i in title_lines:
            draw.text((54, 74 + y_offset), i, font=MEDIUM_FONT, fill=GREEN)
            y_offset += 20
        ...
        draw.rectangle([mark_start, 234, mark_start + bar_width, 240], fill=ORANGE)
        draw.rectangle([mark_start, 217, mark_start + bar_width, 233], fill=ORANGE)
        draw.rectangle([label_start, 217, label_end, 230], fill=GREEN)
...
def display_one(name):
    ...
    draw.text((92, 20 - 7), name_line, font=LARGE_FONT, fill=GREEN)
    draw.rectangle([92, 47, 92 + width(name_line, LARGE_FONT), 47], fill=GREEN)
   
    # location
    draw.text((92, 52), calculate_text(streams[name]['location'], font=MEDIUM_FONT, max_width=223, lines=1)[0], font=MEDIUM_FONT, fill=GREEN)    
    ...
    for i in title_lines:
        draw.text((14, anchor), i, font=LARGE_FONT, fill=GREEN)
    ...
    if info_lines:
        for i in info_lines:
            draw.text((14, anchor), i, font=MEDIUM_FONT, fill=GREEN)
...
def show_volume_overlay(volume):
    ...
    if current_image:
        ...
        draw.rectangle([SCREEN_WIDTH-7, volume_bar_end, SCREEN_WIDTH, SCREEN_HEIGHT], fill=GREEN)
