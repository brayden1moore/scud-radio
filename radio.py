...
def display_everything(direction, name, update=False, readied=False):
    ...
    if readied and not restarting:
        ...
        # marker
        draw.rectangle([mark_start - 1, 227, mark_start + bar_width + 1, 236], fill=ORANGE)
        draw.rectangle([mark_start - mark_width, 230, mark_start - mark_width + bar_width, 233], fill=WHITE)
        draw.rectangle([mark_start + mark_width, 230, mark_start + mark_width + bar_width, 233], fill=WHITE)
