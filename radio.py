...
def display_everything(direction, name, update=False, readied=False, pushed=False):
    ...
    if not restarting:
        ...
        for i in [i for i in stream_list if i not in favorites]:
            draw.rectangle([tick_start, tick_start_y, tick_start + tick_width, tick_start_y + tick_height], fill=readied)
            tick_locations[i] = tick_start
            tick_start += mark_width
...
def show_volume_overlay(volume):
    ...
    if current_image:
        ...
        # volume fill
        draw.rectangle([SCREEN_WIDTH-10, last_volume_bar_end, SCREEN_WIDTH, SCREEN_HEIGHT], fill=BLACK)
