...
def get_stream_list(streams):
    ...
    reruns = [i for i in stream_list if any(j in streams[i]['oneLiner'].lower() for j in ['(r)','re-run','re-wav','restream','playlist','auto dj','night moves', 'night files']) or i=='Monotonic Radio' or ' ARCHIVE' in streams[i]['oneLiner']]
...
def display_everything(direction, name, update=False, readied=False, pushed=False):
    ...
    if readied and not restarting:
        ...
        draw.rectangle([name_chunk_start_x, name_chunk_start - 2, name_chunk_start_x + width(name_line[0], LARGE_FONT_THIN), name_chunk_start + height('S', LARGE_FONT_THIN)], fill=WHITE) # bg
        draw.text((name_chunk_start_x, name_chunk_start - 2), name_line[0], font=LARGE_FONT_THIN, fill=BLACK) 
